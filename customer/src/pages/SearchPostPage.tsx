...
const mockProductTypes = [
    { label: 'Pin xe điện', value: 0 },         // ProductType.ElectricBattery
    { label: 'Ô tô điện', value: 1 },           // ProductType.ElectricCarBattery
    { label: 'Xe máy điện', value: 2 }          // ProductType.ElectricScooterBattery
];
const mockProductTypeLabels = mockProductTypes.map(p => p.label);
...
export const SearchPostPage: React.FC<EcycleCategoryPageProps> = ({ globalSearchTerm }) => {
    ...
    const [activeProductType, setActiveProductType] = useState<ProductType | undefined>(undefined);
    ...
    const handleProductTypeClick = (typeLabel: string) => {
        // Tìm ProductType value tương ứng từ mockProductTypes
        const typeItem = mockProductTypes.find(item => item.label === typeLabel);
        
        if (typeItem) {
            const newType: ProductType = typeItem.value as ProductType;
            setCurrentPage(1); 
    ...
    const handleCloseTypeMenu = () => setTypeMenuAnchorEl(null);
    ...
    const handleProductTypeMenuItemClick = (typeLabel: string) => {
        handleProductTypeClick(typeLabel);
        handleCloseTypeMenu();
    };
    ...
    const handleClearProductTypeFilter = () => {
        setCurrentPage(1);
    ...
    const renderFilterBox = () => (
        <Box sx={{ 
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5, mb: 2 }}>
                ...
                {/* LỌC LOẠI SẢN PHẨM (MỚI) */}
                <Button 
                    variant="outlined" 
                    endIcon={<KeyboardArrowDownIcon />} 
                    sx={{ textTransform: 'none', borderRadius: 2 }}
                    onClick={(event) => setTypeMenuAnchorEl(event.currentTarget)}
                >
                    Loại sản phẩm
                </Button>
                {/* MENU LOẠI SẢN PHẨM */}
                ...
                >
                    {mockProductTypes.map((item) => (
                        <MenuItem 
                            key={item.value} 
                            onClick={() => handleProductTypeMenuItemClick(item.label)}
                            selected={activeProductType === item.value}
                        >
                            {item.label}
                        </MenuItem>
                    ))}
                ...
                {/* HIỂN THỊ CHIP LỌC LOẠI SẢN PHẨM ĐANG HOẠT ĐỘNG (MỚI) */}
                {activeProductType !== undefined && (
                    <Chip
                        label={mockProductTypes.find(t => t.value === activeProductType)?.label || 'Loại xe'}
                        onDelete={handleClearProductTypeFilter} 
                        deleteIcon={<CloseIcon />}
                        color="secondary"
                        variant="outlined"
                        sx={{ textTransform: 'none', fontWeight: 'medium' }}
        ...
        <Box sx={{ flexGrow: 1, pb: 4, bgcolor: '#f5f5f5' }}>
            ...
            <Container maxWidth="lg" sx={{ pt: 3 }}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                    ...
                    <Link to="/search-post" style={{ textDecoration: 'text.primary', color: 'black', fontWeight: 'bold', }}>
                        <SearchIcon sx={{ fontSize: 16, verticalAlign: 'middle', mr: 0.5 }} /> Trang tìm kiếm
                    </Link>
                </Typography>
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                        ...
                        <Box sx={{ 
                            ...
                            <Box sx={{ p: 0, position: 'sticky', top: 80 }}>
                                {/* 💡 LỌC THEO LOẠI SẢN PHẨM (SỬ DỤNG DỮ LIỆU TYPE MỚI) */}
                                <FilterSection 
                                    title="Lọc theo loại sản phẩm" 
                                    items={mockProductTypeLabels} // 💡 Dữ liệu cho sidebar
                                    onItemClick={handleProductTypeClick} // 💡 Sử dụng handler mới
                                />
                                <Divider sx={{ mb: 2 }} />
