import React, { useState, useEffect, useMemo, type JSX, useRef } from 'react';
import { 
    Box, Typography, Paper, useTheme, Stack, 
    Divider, Chip, Button, Alert, List, ListItem, ListItemText, ListItemIcon,
    CircularProgress, Snackbar, TextField, Dialog, DialogTitle, DialogContent, DialogActions
} from '@mui/material';
...
import AttachMoneyIcon from '@mui/icons-material/AttachMoney'; // Icon cho Transaction ID


// ====================================================================
// IMPORT SERVICE VÀ ENUMS (Cập nhật Interface)
// ====================================================================

// Giả định các hàm và Enums được import từ file services/auctionService.ts
import { 
    AuctionStatusValue, type AuctionStatus, 
    DepositStatusValue, type DepositStatus, 
...
// ====================================================================
// Interface này nên được đặt trong file service/auctionService.ts (nhưng tôi đặt lại ở đây để minh họa)
...
const getStatusChip = (status: AuctionStatus): JSX.Element => {
    switch (status) {
        ...
        case AuctionStatusValue.Completed:
            return <Chip label="Hoàn tất" color="primary" icon={<CheckCircleIcon style={{ fontSize: 16 }} />} size="small" />;
...
const fetchAuctionDetailWithBids = async (idNumber: number): Promise<AuctionDetailWithBids> => {
    ...
    const auctionDetailPromise = getAuctionDetail(idNumber) as Promise<AuctionDetailData>; // Sử dụng interface đã cập nhật
...
const AuctionDetailPage: React.FC = () => {
    ...
    const handleUpdateStatus = async (
        ...
        else if (newStatus === AuctionStatusValue.Completed) actionName = 'Hoàn tất';
        else if (newStatus === AuctionStatusValue.Cancelled) actionName = 'Hủy';

        if (newStatus !== AuctionStatusValue.Completed && 
            !window.confirm(`Bạn có chắc chắn muốn ${actionName} đấu giá ID ${auction.auctionId} không?`)) {
            return;
        }
        ...
        try {
            if (newStatus === AuctionStatusValue.Completed) {
                if (!transactionId || transactionId <= 0) {
                    throw new Error("Transaction ID không hợp lệ.");
                }
                await updateAuctionCompleteStatusApi(auction.auctionId, transactionId);
            } else {
    ...
    const handleConfirmComplete = () => {
        const txId = parseInt(transactionIdInput);
        if (isNaN(txId) || txId <= 0) {
            setSnackbar({ open: true, message: 'Vui lòng nhập Transaction ID hợp lệ.', severity: 'error' });
            return;
        }
        handleUpdateStatus(AuctionStatusValue.Completed, txId);
    };
    ...
    const isAuctionCompleted = auction.status === AuctionStatusValue.Completed;
    ...
    return (
        <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={4}>
                ...
                <Box sx={{ width: { xs: '100%', md: '58.33%' } }}>
                    ...
                    <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
                        ...
                        <Stack direction="row" flexWrap="wrap" spacing={2} rowGap={2} sx={{ mb: 2 }}>
                            ...
                            {/* CẬP NHẬT: Hiển thị Người thắng và Transaction ID */}
                            {(isAuctionEnded && auction.winnerId) || isAuctionCompleted ? (
                                <Box sx={{ width: '100%', mt: 3 }}>
                                    <Alert severity={isAuctionCompleted ? 'success' : 'info'} icon={<CheckCircleIcon />}>
                                        <Typography fontWeight="bold" sx={{ mb: 1 }}>
                                            {isAuctionCompleted ? 'KẾT QUẢ ĐÃ HOÀN TẤT' : 'KẾT QUẢ TẠM THỜI (Đã kết thúc)'}:
                                        </Typography>
                                        ...
                                        {/* Transaction ID (Link - Chỉ khi Completed) */}
                                        {isAuctionCompleted && auction.transactionId && (
                                            <Stack direction="row" alignItems="center" spacing={1} sx={{ mt: 1 }}>
                                                ...
                                                <Typography variant="body1">
                                                    ...
                                                    <Button 
                                                        component={Link} 
                                                        to={`/transactions/${auction.transactionId}`} 
                                                        size="small" 
                                                        sx={{ textTransform: 'none', px: 1 }}
                                                        color="success"
                                                    >
                                        ...
                                        {isAuctionCompleted && !auction.transactionId && (
                                            <Typography variant="body2" color="error" sx={{ mt: 1 }}>
                                                **Lỗi:** Đấu giá Hoàn tất nhưng thiếu Transaction ID!
                                            </Typography>
                                        )}
                                    </Alert>
                                </Box>
                ...
                >
                    <Paper sx={{ p: 3, boxShadow: theme.shadows[5] }}>
                        ...
                        <Alert 
                            severity={
                                isAuctionCompleted ? 'success' :
                                auction.status === AuctionStatusValue.Ended ? 'info' : 
                                (auction.status === AuctionStatusValue.Pending ? 'warning' : 'error')
                            }
                        >
                            <Typography fontWeight="bold">
                                {isAuctionActive ? `Đang diễn ra: ${timeLeft}` : (
                                    isAuctionCompleted ? 'Phiên đấu giá đã HOÀN TẤT.' :
                                    auction.status === AuctionStatusValue.Ended ? 'Phiên đấu giá đã KẾT THÚC, chờ xử lý hoàn tất.' :
                                    auction.status === AuctionStatusValue.Pending ? 'Phiên đang CHỜ DUYỆT.' : 
                                    'Phiên đã bị HỦY.'
                                )}
                            </Typography>
                        </Alert>
                        ...
                        {/* NÚT HOÀN TẤT (COMPLETE) - Mở Dialog */}
                        <Button 
                            variant="contained" 
                            color="success" 
                            fullWidth 
                            ...
                            disabled={actionLoading || auction.status !== AuctionStatusValue.Ended || !auction.winnerId || isAuctionCompleted}
                            startIcon={<DoneAllIcon />}
                        >
                            HOÀN TẤT Đấu giá (Complete)
                        </Button>
                        ...
                        <Button 
                            ...
                            disabled={actionLoading || isAuctionActive || isAuctionEnded}
                            startIcon={actionLoading && auction.status !== AuctionStatusValue.Active ? <CircularProgress size={24} color="inherit" /> : <AccessTimeIcon />}
                        >
                        ...
                        <Button 
                            ...
                            disabled={actionLoading || auction.status === AuctionStatusValue.Pending || isAuctionEnded}
                            startIcon={actionLoading && auction.status !== AuctionStatusValue.Pending ? <CircularProgress size={24} color="inherit" /> : <AutorenewIcon />}
                        >
                        ...
                        <Button 
                            ...
                            disabled={actionLoading || auction.status === AuctionStatusValue.Cancelled || isAuctionCompleted}
                            startIcon={actionLoading && auction.status !== AuctionStatusValue.Cancelled ? <CircularProgress size={24} color="inherit" /> : <BlockIcon />}
                        >
                        ...
                        <Button 
                            ...
                            startIcon={actionLoading ? <CircularProgress size={24} color="inherit" /> : <DeleteForeverIcon />}
                        >
            ...
            {/* ======================================= */}
            {/* DIALOG XÁC NHẬN HOÀN TẤT & TRANSACTION ID */}
            {/* ======================================= */}
            <Dialog 
            ...
            >
                <DialogTitle>
                    <Stack direction="row" alignItems="center" spacing={1}>
                        ...
                        <Typography variant="h6">Hoàn Tất Đấu Giá #{auction?.auctionId}</Typography>
                    </Stack>
                </DialogTitle>
                <DialogContent dividers>
                    <Alert severity="info" sx={{ mb: 2 }}>
                        Vui lòng nhập **Transaction ID** đã được tạo để xác nhận hoàn tất (Completed) đấu giá.
                    </Alert>
                    <TextField
                        autoFocus
                        margin="dense"
                ...
                <DialogActions>
                    <Button onClick={() => setOpenCompleteDialog(false)} color="error" disabled={actionLoading}>
                        Hủy
                    </Button>
                    <Button 
                        ...
                        color="success" 
                        variant="contained"
                        disabled={actionLoading || !transactionIdInput || parseInt(transactionIdInput) <= 0}
